 ...
 * 拒绝（rc.2：未收录类型落盘会让会话下次加载被持久化层拒绝）。
 *
 * 拒绝**不再静默**：首次遇到某会话的某个被拒类型时打一次可见告警（会话维度去重，
 * 同一会话同一类型只报一次），说明审计事件按文档所述不落盘、会话因此保持可加载。
 * @param {ReadonlySet<string>} knownTypes - KNOWN_SESSION_EVENT_TYPES。
 * @param {boolean} [ignorableAppend] - 宿主 append 是否盖章 ignorable 信封。
 * @param {(message: string) => void} [warn] - 可见告警出口（缺省静默，测试可注入）。
 * @returns {(type: string, sessionKey?: string) => {append: boolean, ignorable: boolean}} 决策函数。
 */
export function makeEventGate(knownTypes, ignorableAppend = false, warn = () => {}) {
  ...
  return (type, sessionKey) => {
    ...
    const key = `${sessionKey ?? 'unknown-session'}:${type}`
    if (!warned.has(key)) {
      warned.add(key)
      warn(`${PLUGIN_NAME} session event ${type} is not recorded by this host and cannot carry the ignorable envelope: the audit event is skipped so the session keeps loading (documented limitation, not a silent failure).`)
    }
 ...
 * @param {(type: string, sessionKey?: string) => {append: boolean, ignorable: boolean}} gate - makeEventGate 产物。
...
export function maybeAppendSessionEvent(session, type, data, gate, warn) {
  ...
  const decision = gate(type, session.id)
