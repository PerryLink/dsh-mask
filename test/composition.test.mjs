...
// @module dsh-mask/test/composition.test
...
function configFor(pluginRow, storageRoot, configLines = []) {
  return [
...
const temporaryRoot = mkdtempSync(join(tmpdir(), 'dsh-mask-loader-'))
...
test('Loader composition mounts the plugin and applies its default config', () => {
  ...
  writeFileSync(configPath, configFor(pathToFileURL(entry).href, storageRoot))
  ...
  assert.ok(summary.commands.includes('mask'))
  assert.equal(summary.maskTestTool, true)
  assert.match(summary.statusText, /mask: enabled/u)
})
...
test('Loader composition applies the special config value (registerTools: false)', () => {
  ...
  const marker = evidence.stdout.match(/DSH_LOADER_RESULT (.+)$/mu)
  const summary = JSON.parse(marker[1])
  assert.equal(summary.maskTestTool, false)
  assert.ok(summary.commands.includes('mask'), '/mask stays registered when only the tool is disabled')
})
...
test('invalid config fails loud through the Loader for the expected reason', () => {
  ...
  for (const entryCase of cases) {
    ...
    writeFileSync(configPath, configFor(entryUrl, storageRoot, entryCase.lines))
    const evidence = runRunner(configPath, 'tool')
