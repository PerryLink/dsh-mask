...
for (const file of FILES) {
  ...
  const checks = [
    ...
    ['install gh url', text.includes('git+https://github.com/PerryLink/dsh-mask.git')],
    ['license link', text.includes('[LICENSE](LICENSE)')],
    ['command token', /`\/mask`/.test(text)],
    ['mask test tool token', /`mask_test`/.test(text)],
    ['config mode token', /`mode`/.test(text)],
    ['config entities token', /`entities`/.test(text)],
    ['config persist token', /`persistRestoreTable`/.test(text)],
